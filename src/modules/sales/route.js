...
router
  ...
  .post(authorizeRoles("OWNER", "ADMIN", "MANAGER"), createSaleValidator, salesController.createSale);

router.get("/:identifier", salesController.getSaleById);
router.post(
  "/:id/return",
  [authorizeRoles("OWNER", "ADMIN", "MANAGER"), ...processReturnValidator],
  salesController.processReturn
);
router.get("/:id/returns", salesController.getReturnHistory);
router.post("/:id/void", [authorizeRoles("OWNER", "ADMIN", "MANAGER"), ...voidSaleValidator], salesController.voidSale);
router.get("/returns/all", [authorizeRoles("OWNER", "ADMIN", "MANAGER")], salesController.getAllReturns);
